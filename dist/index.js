...
export default function MaterialReveal({
  ...
  const [classList, setClassList] = useState(['material-reveal']); // main states

  const [isActive, setIsActive] = useState(false);
  const [isOpen, setIsOpen] = useState(false);
  const [isOpenLast, setIsOpenLast] = useState(false);
  const [startPos, setStartPos] = useState(null); // element refs

  const infoPanel = useRef(null);
  const buttonPanel = useRef(null); // set up classes
  ...
  }, [className, isActive, variant]); // Add event listener

  useEffect(() => {
    const {
      current: info
    } = infoPanel;
    if (info === null) return; // when mouse is down set contol class and mark as active
    ...
    info.addEventListener('keyup', onKeyUp); // mouse listeners
  ...
  return <StyledDiv className={classList.join(' ')}>
      ...
      <div className="material-reveal__options">
        <span ref={buttonPanel}>
          {actions.map(action => <button title={action.title} aria-label={action.title} onClick={() => action.action}>
              {action.icon}
            </button>)}
        </span>
      </div>
